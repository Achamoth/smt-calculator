export const SMT4A_SKILL_DATA = {
  "5.67 Billion Hands": {
    cost: 1050,
    effect: "Weak dmg, Smirk: Increased power",
    element: "alm",
    rank: 0,
    target: "All foes",
  },
  "Acid Breath": {
    cost: 1065,
    effect: "Rakunda + Sukunda",
    element: "sup",
    rank: 17,
    target: "All foes",
  },
  "Adaptive Tactics": {
    cost: 1036,
    effect: "Change weakness if exploited",
    element: "sup",
    rank: 0,
  },
  Agi: {
    cost: 1005,
    effect: "Weak dmg",
    element: "fir",
    rank: 1,
    target: "1 foe",
  },
  Agidyne: {
    cost: 1020,
    effect: "Heavy dmg",
    element: "fir",
    rank: 19,
    target: "1 foe",
  },
  Agilao: {
    cost: 1010,
    effect: "Medium dmg",
    element: "fir",
    rank: 7,
    target: "1 foe",
  },
  "Akasha Arts": {
    cost: 1030,
    effect: "Severe dmg, Smirk: Pierce",
    element: "phy",
    rank: 31,
    target: "1 foe",
  },
  "Alluring Banter": {
    cost: 1036,
    effect: "Charm, Minimize defense",
    element: "ail",
    rank: 0,
    target: "1 foe",
  },
  "Ally Counter": {
    effect:
      "May perform a weak attack when an ally is struck by a phys/gun attack",
    element: "pas",
    rank: 15,
  },
  "Ally Retaliate": {
    effect:
      "May perform a heavy attack when an ally is struck by a phys/gun attack",
    element: "pas",
    rank: 23,
  },
  "Almighty Pleroma": {
    effect: "Almighty damage +20%",
    element: "pas",
    rank: 30,
  },
  Amrita: {
    cost: 1020,
    effect: "Cures all ailments",
    element: "rec",
    rank: 23,
    target: "1 ally",
  },
  Andalusia: {
    cost: 1020,
    effect: "Heavy dmg, 4-12 hits",
    element: "phy",
    rank: 0,
    target: "Multi foes",
  },
  Antichthon: {
    cost: 1110,
    effect: "Severe dmg, Smirk: Debilitate",
    element: "alm",
    rank: 31,
    target: "All foes",
  },
  "Attack Knowhow": {
    effect: "Enables use of attack items",
    element: "pas",
    rank: 7,
  },
  "Axel Claw": {
    cost: 1012,
    effect: "Medium dmg, 1-3 hits",
    element: "phy",
    rank: 13,
    target: "Multi foes",
  },
  "Babylon Goblet": {
    cost: 1070,
    effect: "Medium dmg, Panic",
    element: "alm",
    rank: 0,
    target: "All foes",
  },
  "Bad Company": {
    cost: 1011,
    effect: "Summons your highest level demons from stock",
    element: "oth",
    rank: 6,
  },
  "Beastly Reaction": {
    effect: "Increase hit/evade rate",
    element: "pas",
    rank: 8,
  },
  "Berserker God": {
    cost: 1013,
    effect: "Heavy dmg",
    element: "phy",
    rank: 21,
    target: "1 foe",
  },
  "Bind Voice": {
    cost: 1020,
    effect: "Bind",
    element: "ail",
    rank: 10,
    target: "All foes",
  },
  "Binding Claw": {
    cost: 1011,
    effect: "Medium dmg, Bind",
    element: "phy",
    rank: 3,
    target: "1 foe",
  },
  "Blade of Terror": {
    cost: 1035,
    effect: "Heavy dmg, 1-5 hits, Pierce, Panic",
    element: "phy",
    rank: 0,
    target: "Multi foes",
  },
  "Blank Bullet": {
    cost: 1022,
    effect: "Medium dmg, 2 hits, Panic",
    element: "gun",
    rank: 0,
    target: "All foes",
  },
  "Blast Arrow": {
    cost: 1019,
    effect: "Medium dmg",
    element: "gun",
    rank: 17,
    target: "All foes",
  },
  Blight: {
    cost: 1018,
    effect: "Weak dmg, Poison",
    element: "phy",
    rank: 10,
    target: "All foes",
  },
  "Blink of Death": {
    cost: 1030,
    effect: "Heavy dmg, Rakunda, Smirk: 100% kill",
    element: "dar",
    rank: 0,
    target: "1 foe",
  },
  "Blood Ritual": {
    cost: 1025,
    effect: "Buff all stats once, HP falls to 1",
    element: "sup",
    rank: 21,
  },
  "Bloody Glee": {
    effect: "Increase critical hit rate",
    element: "pas",
    rank: 17,
  },
  "Bouncing Claw": {
    cost: 1004,
    effect: "Weak dmg, 1-3 hits",
    element: "phy",
    rank: 3,
    target: "1 foe",
  },
  Breath: {
    cost: 1024,
    effect: "Medium dmg, 1-5 hits",
    element: "ice",
    rank: 0,
    target: "Multi foes",
  },
  Bufu: {
    cost: 1005,
    effect: "Weak dmg",
    element: "ice",
    rank: 1,
    target: "1 foe",
  },
  Bufudyne: {
    cost: 1020,
    effect: "Heavy dmg",
    element: "ice",
    rank: 19,
    target: "1 foe",
  },
  Bufula: {
    cost: 1010,
    effect: "Medium dmg",
    element: "ice",
    rank: 7,
    target: "1 foe",
  },
  "Carol Hit": {
    effect: "Weak dmg, 50% Lost",
    element: "phy",
    rank: 0,
    target: "1 foe",
  },
  "Chakra Walk": {
    effect: "Regain MP while walking. Inactive while in stock.",
    element: "pas",
    rank: 13,
  },
  Charge: {
    cost: 1009,
    effect: "Next Phys/Gun attack x2.5 damage",
    element: "sup",
    rank: 15,
  },
  Chariot: {
    cost: 1060,
    effect: "Medium dmg, Sukunda",
    element: "alm",
    rank: 0,
    target: "All foes",
  },
  "Cold World": {
    cost: 1070,
    effect: "Heavy dmg, Instant kill",
    element: "ice",
    rank: 27,
    target: "All foes",
  },
  "Combat Tara": {
    cost: 1060,
    effect: "Buff all stats once",
    element: "sup",
    rank: 0,
    target: "All allies",
  },
  Concentrate: {
    cost: 1011,
    effect: "Next magic attack x2.5 damage",
    element: "sup",
    rank: 23,
  },
  Cough: {
    cost: 1004,
    effect: "Sick",
    element: "ail",
    rank: 2,
    target: "1 foe",
  },
  Counter: {
    effect: "May perform a weak attack when struck by a phys/gun attack",
    element: "pas",
    rank: 6,
  },
  "Critical Eye": {
    cost: 1011,
    effect: "Next Phys/Gun attack guaranteed crit",
    element: "sup",
    rank: 17,
  },
  "Critical Wave": {
    cost: 1012,
    effect: "Weak dmg, High crit/low hit",
    element: "phy",
    rank: 5,
    target: "All foes",
  },
  "Damascus Claw": {
    cost: 1007,
    effect: "Medium dmg, 1-3 hits",
    element: "phy",
    rank: 7,
    target: "1 foe",
  },
  "Dance of Mara": {
    cost: 1020,
    effect: "Charm",
    element: "ail",
    rank: 0,
    target: "All foes",
  },
  "Dark Grudge": {
    cost: 1030,
    effect: "Heavy dmg, Smirk: 100% kill",
    element: "dar",
    rank: 0,
    target: "1 foe",
  },
  "Dark Pierce": {
    effect: "Dark attacks bypass resistances",
    element: "pas",
    rank: 31,
  },
  "Dark Pleroma": {
    effect: "Dark damage +25%",
    element: "pas",
    rank: 16,
  },
  "Dark Sword": {
    cost: 1016,
    effect: "Medium dmg, 2 hits, Mute",
    element: "phy",
    rank: 21,
    target: "1 foe",
  },
  "Dazzle Ray": {
    cost: 1035,
    effect: "Severe dmg, Smirk: 100% kill",
    element: "lig",
    rank: 0,
    target: "1 foe",
  },
  "Deadly Wind": {
    cost: 1040,
    effect: "Severe dmg, Smirk: Pierce",
    element: "for",
    rank: 29,
    target: "1 foe",
  },
  "Death Lust": {
    cost: 1070,
    effect: "Weak dmg, Charm",
    element: "alm",
    rank: 0,
    target: "All foes",
  },
  "Death's Door": {
    cost: 1035,
    effect: "Weak dmg, HP to 1 for sick targets",
    element: "alm",
    rank: 21,
    target: "All foes",
  },
  Debilitate: {
    cost: 1100,
    effect: "Debuff all stats once",
    element: "sup",
    rank: 28,
    target: "All foes",
  },
  Dekaja: {
    cost: 1030,
    effect: "Removes all buffs",
    element: "sup",
    rank: 23,
    target: "All foes",
  },
  Dekunda: {
    cost: 1030,
    effect: "Removes all debuffs",
    element: "sup",
    rank: 23,
    target: "All allies",
  },
  "Desperate Hit": {
    cost: 1040,
    effect: "Weak dmg, 1-5 hits",
    element: "alm",
    rank: 0,
    target: "Multi foes",
  },
  Dia: {
    cost: 1008,
    effect: "Restores small amount of HP",
    element: "rec",
    rank: 2,
    target: "1 ally",
  },
  Diarahan: {
    cost: 1030,
    effect: "Restores all HP",
    element: "rec",
    rank: 20,
    target: "1 ally",
  },
  Diarama: {
    cost: 1016,
    effect: "Restores large amount of HP",
    element: "rec",
    rank: 8,
    target: "1 ally",
  },
  "Die For Me!": {
    cost: 1056,
    effect: "Severe dmg, Smirk: 80% kill",
    element: "dar",
    rank: 0,
    target: "All foes",
  },
  Doping: {
    cost: 1100,
    effect: "+30% max HP",
    element: "sup",
    rank: 28,
    target: "All allies",
  },
  Dormina: {
    cost: 1004,
    effect: "Sleep",
    element: "ail",
    rank: 2,
    target: "1 foe",
  },
  "Draconic Reaction": {
    effect: "Greatly increase hit/evade rate",
    element: "pas",
    rank: 21,
  },
  "Drain Dark": {
    effect: "Absorbs dark damage",
    element: "pas",
    rank: 29,
  },
  "Drain Elec": {
    effect: "Absorbs elec damage",
    element: "pas",
    rank: 29,
  },
  "Drain Fire": {
    effect: "Absorbs fire damage",
    element: "pas",
    rank: 29,
  },
  "Drain Force": {
    effect: "Absorbs force damage",
    element: "pas",
    rank: 29,
  },
  "Drain Gun": {
    effect: "Absorbs gun damage",
    element: "pas",
    rank: 29,
  },
  "Drain Ice": {
    effect: "Absorbs ice damage",
    element: "pas",
    rank: 29,
  },
  "Drain Light": {
    effect: "Absorbs light damage",
    element: "pas",
    rank: 29,
  },
  "Drain Phys": {
    effect: "Absorbs physical damage",
    element: "pas",
    rank: 29,
  },
  "Dream Fist": {
    cost: 1009,
    effect: "Weak dmg, Sleep",
    element: "phy",
    rank: 3,
    target: "1 foe",
  },
  "Dream Needle": {
    cost: 1009,
    effect: "Weak dmg, Sleep",
    element: "gun",
    rank: 3,
    target: "1 foe",
  },
  "Dream Raga": {
    cost: 1065,
    effect: "Sleep/Panic/Charm, Rakunda x2",
    element: "ail",
    rank: 0,
    target: "All foes",
  },
  Earthquake: {
    cost: 1025,
    effect: "Medium dmg, Daze",
    element: "phy",
    rank: 20,
    target: "All foes",
  },
  "Eat Whole": {
    cost: 1014,
    effect: "Weak dmg, Absorbs HP",
    element: "phy",
    rank: 23,
    target: "All foes",
  },
  "Elec Pierce": {
    effect: "Elec attacks bypass resistances",
    element: "pas",
    rank: 31,
  },
  "Elec Pleroma": {
    effect: "Elec damage +25%",
    element: "pas",
    rank: 16,
  },
  Endure: {
    effect: "Revive from death once per battle with 1 HP",
    element: "pas",
    rank: 16,
  },
  "Enduring Cheer": {
    cost: 1030,
    effect: "Medium chance of Endure this turn.",
    element: "sup",
    rank: 0,
    target: "All allies",
  },
  "Enduring Soul": {
    effect: "Revive from death once per battle with full HP",
    element: "pas",
    rank: 28,
  },
  "Energy Drain": {
    cost: 1007,
    effect: "Weak dmg, Absorbs HP/MP",
    element: "alm",
    rank: 23,
    target: "1 foe",
  },
  Enlightenment: {
    cost: 1008,
    effect: "Drain weaknesses, buff all stats if successful",
    element: "sup",
    rank: 0,
  },
  Estoma: {
    cost: 1021,
    effect: "Avoid battles with low-level demons while skill is in effect",
    element: "oth",
    rank: 6,
  },
  "Eternal Rest": {
    cost: 1050,
    effect: "N/A dmg, Instant kill sleeping targets",
    element: "alm",
    rank: 23,
    target: "All foes",
  },
  "Evil Melody": {
    cost: 1045,
    effect: "Weak dmg, Smirk: 100% kill",
    element: "alm",
    rank: 0,
    target: "1 foe",
  },
  "Evil Shine": {
    cost: 1110,
    effect: "Severe dmg, Panic",
    element: "alm",
    rank: 0,
    target: "All foes",
  },
  "Fang Breaker": {
    cost: 1012,
    effect: "Weak dmg, Tarunda",
    element: "phy",
    rank: 7,
    target: "1 foe",
  },
  "Fatal Sword": {
    cost: 1008,
    effect: "Medium dmg",
    element: "phy",
    rank: 7,
    target: "1 foe",
  },
  "Fear Darkness": {
    cost: 1035,
    effect: "Severe dmg, Smirk: 100% kill",
    element: "dar",
    rank: 0,
    target: "1 foe",
  },
  "Fire Breath": {
    cost: 1021,
    effect: "Weak dmg, 1-4 hits",
    element: "fir",
    rank: 9,
    target: "Multi foes",
  },
  "Fire Pierce": {
    effect: "Fire attacks bypass resistances",
    element: "pas",
    rank: 31,
  },
  "Fire Pleroma": {
    effect: "Fire damage +25%",
    element: "pas",
    rank: 16,
  },
  "Fire of Lethargy": {
    cost: 1045,
    effect: "Heavy dmg, Rakunda + Sukunda",
    element: "fir",
    rank: 0,
    target: "1 foe",
  },
  "Fire of Sinai": {
    cost: 1055,
    effect: "Severe dmg, 1-5 hits",
    element: "alm",
    rank: 0,
    target: "Multi foes",
  },
  "Floral Gust": {
    cost: 1060,
    effect: "Medium dmg, 1-4 hits",
    element: "for",
    rank: 27,
    target: "Multi foes",
  },
  "Fog Breath": {
    cost: 1065,
    effect: "Tarunda + Sukunda",
    element: "sup",
    rank: 17,
    target: "All foes",
  },
  "Force Pierce": {
    effect: "Force attacks bypass resistances",
    element: "pas",
    rank: 31,
  },
  "Force Pleroma": {
    effect: "Force damage +25%",
    element: "pas",
    rank: 16,
  },
  "Frenzied Chomp": {
    cost: 1006,
    effect: "Weak dmg, Poison/Bind/Charm",
    element: "phy",
    rank: 7,
    target: "1 foe",
  },
  "Glacial Blast": {
    cost: 1060,
    effect: "Medium dmg, 1-4 hits",
    element: "ice",
    rank: 28,
    target: "Multi foes",
  },
  "God's Bow": {
    cost: 1040,
    effect: "Heavy dmg, 100% kill",
    element: "lig",
    rank: 0,
    target: "1 foe",
  },
  "Gram Slice": {
    cost: 1005,
    effect: "Weak dmg",
    element: "phy",
    rank: 1,
    target: "1 foe",
  },
  "Grand Tack": {
    cost: 1011,
    effect: "Heavy dmg",
    element: "gun",
    rank: 19,
    target: "1 foe",
  },
  "Great Logos": {
    cost: 1100,
    effect: "Severe dmg",
    element: "alm",
    rank: 30,
    target: "All foes",
  },
  "Growing Hate": {
    cost: 1100,
    effect: "Severe dmg, Stronger vs dark resistance",
    element: "alm",
    rank: 0,
    target: "All foes",
  },
  "Gun Pierce": {
    effect: "Gun attacks bypass resistances",
    element: "pas",
    rank: 31,
  },
  "Gun Pleroma": {
    effect: "Gun damage +25%",
    element: "pas",
    rank: 25,
  },
  Gungnir: {
    cost: 1022,
    effect: "Severe dmg, Pierce",
    element: "phy",
    rank: 0,
    target: "1 foe",
  },
  "Hades Blast": {
    cost: 1042,
    effect: "Heavy dmg",
    element: "phy",
    rank: 23,
    target: "All foes",
  },
  Hama: {
    cost: 1006,
    effect: "Weak dmg, Smirk: 30% kill",
    element: "lig",
    rank: 3,
    target: "1 foe",
  },
  Hamaon: {
    cost: 1012,
    effect: "Heavy dmg, Smirk: 55% kill",
    element: "lig",
    rank: 13,
    target: "1 foe",
  },
  "Hard Worker": {
    effect: "Gain more EXP from battles",
    element: "pas",
    rank: 6,
  },
  "Haunting Rhapsody": {
    cost: 1100,
    effect: "Panic, Debilitate",
    element: "ail",
    rank: 0,
    target: "All foes",
  },
  "Head Crush": {
    cost: 1006,
    effect: "Weak dmg, Daze",
    element: "phy",
    rank: 3,
    target: "1 foe",
  },
  "Heal Pleroma": {
    effect: "Healing +20%",
    element: "pas",
    rank: 16,
  },
  "Healing Knowhow": {
    effect: "Enables use of healing items",
    element: "pas",
    rank: 8,
  },
  "Heat Wave": {
    cost: 1013,
    effect: "Weak dmg",
    element: "phy",
    rank: 5,
    target: "All foes",
  },
  "Heaven's Bow": {
    cost: 1033,
    effect: "Heavy dmg",
    element: "gun",
    rank: 22,
    target: "All foes",
  },
  "Hell Thrust": {
    cost: 1028,
    effect: "Heavy dmg, 2-4 hits, Smirk: Rakunda",
    element: "phy",
    rank: 29,
    target: "Multi foes",
  },
  "Hellish Brand": {
    cost: 1020,
    effect: "Heavy dmg, Dekaja",
    element: "fir",
    rank: 0,
    target: "1 foe",
  },
  "Hellish Mask": {
    effect: "Resists all ailments",
    element: "pas",
    rank: 17,
  },
  "High Dark Pleroma": {
    effect: "Dark damage +50%",
    element: "pas",
    rank: 25,
  },
  "High Elec Pleroma": {
    effect: "Elec damage +50%",
    element: "pas",
    rank: 25,
  },
  "High Fire Pleroma": {
    effect: "Fire damage +50%",
    element: "pas",
    rank: 25,
  },
  "High Force Pleroma": {
    effect: "Force damage +50%",
    element: "pas",
    rank: 25,
  },
  "High Gun Pleroma": {
    effect: "Gun damage +50%",
    element: "pas",
    rank: 29,
  },
  "High Heal Pleroma": {
    effect: "Healing +50%",
    element: "pas",
    rank: 25,
  },
  "High Ice Pleroma": {
    effect: "Ice damage +50%",
    element: "pas",
    rank: 25,
  },
  "High Light Pleroma": {
    effect: "Light damage +50%",
    element: "pas",
    rank: 25,
  },
  "High Phys Pleroma": {
    effect: "Phys damage +50%",
    element: "pas",
    rank: 29,
  },
  "Holy Wrath": {
    cost: 1050,
    effect: "Medium dmg, Strong vs Chaos",
    element: "alm",
    rank: 20,
    target: "All foes",
  },
  "Ice Age": {
    cost: 1040,
    effect: "Severe dmg, Smirk: Pierce",
    element: "ice",
    rank: 29,
    target: "1 foe",
  },
  "Ice Breath": {
    cost: 1021,
    effect: "Weak dmg, 1-4 hits",
    element: "ice",
    rank: 9,
    target: "Multi foes",
  },
  "Ice Pierce": {
    effect: "Ice attacks bypass resistances",
    element: "pas",
    rank: 31,
  },
  "Ice Pleroma": {
    effect: "Ice damage +25%",
    element: "pas",
    rank: 16,
  },
  "Imposing Stance": {
    cost: 1050,
    effect: "Removes 1 Press Turn icon on foe's next turn",
    element: "oth",
    rank: 23,
  },
  "Infernal Hail": {
    cost: 1030,
    effect: "Medium dmg, Sukunda",
    element: "ice",
    rank: 0,
    target: "All foes",
  },
  Invitation: {
    cost: 1065,
    effect: "Revives, fully heals, and summons a demon from stock",
    element: "oth",
    rank: 30,
  },
  "Iron Judgement": {
    cost: 1008,
    effect: "Medium dmg",
    element: "phy",
    rank: 0,
    target: "1 foe",
  },
  "Javelin Rain": {
    cost: 1025,
    effect: "Medium dmg",
    element: "phy",
    rank: 17,
    target: "All foes",
  },
  Judgement: {
    cost: 1050,
    effect: "Medium dmg, Strong vs Neutral",
    element: "alm",
    rank: 20,
    target: "All foes",
  },
  "Judgement Light": {
    cost: 1056,
    effect: "Severe dmg, Smirk: 80% kill",
    element: "lig",
    rank: 23,
    target: "All foes",
  },
  "King Bufula": {
    cost: 1024,
    effect: "Medium dmg, Smirk: Rakunda",
    element: "ice",
    rank: 0,
    target: "All foes",
  },
  "Life Aid": {
    effect: "Restore large amount of HP after battle",
    element: "pas",
    rank: 15,
  },
  "Life Bonus": {
    effect: "Maximum HP +10%",
    element: "pas",
    rank: 2,
  },
  "Life Drain": {
    cost: 1005,
    effect: "Weak dmg, Absorbs HP",
    element: "alm",
    rank: 1,
    target: "1 foe",
  },
  "Life Gain": {
    effect: "Maximum HP +20%",
    element: "pas",
    rank: 13,
  },
  "Life Surge": {
    effect: "Maximum HP +30%",
    element: "pas",
    rank: 20,
  },
  "Light Devourer": {
    cost: 1033,
    effect: "Heavy dmg, Absorbs HP",
    element: "dar",
    rank: 0,
    target: "All foes",
  },
  "Light Life Aid": {
    effect: "Restore slight HP after battle",
    element: "pas",
    rank: 4,
  },
  "Light Mana Aid": {
    effect: "Restore slight MP after battle",
    element: "pas",
    rank: 4,
  },
  "Light Pierce": {
    effect: "Light attacks bypass resistances",
    element: "pas",
    rank: 31,
  },
  "Light Pleroma": {
    effect: "Light damage +25%",
    element: "pas",
    rank: 16,
  },
  "Loyalty Slash": {
    cost: 1020,
    effect: "Heavy dmg, Smirk: 100% kill",
    element: "phy",
    rank: 0,
    target: "1 foe",
  },
  Lullaby: {
    cost: 1011,
    effect: "Sleep",
    element: "ail",
    rank: 10,
    target: "All foes",
  },
  Lunge: {
    cost: 1006,
    effect: "Weak dmg, High crit/low hit",
    element: "phy",
    rank: 1,
    target: "1 foe",
  },
  "Luster Candy": {
    cost: 1080,
    effect: "Buff all stats once",
    element: "sup",
    rank: 27,
    target: "All allies",
  },
  Mabufu: {
    cost: 1012,
    effect: "Weak dmg",
    element: "ice",
    rank: 5,
    target: "All foes",
  },
  Mabufudyne: {
    cost: 1048,
    effect: "Heavy dmg",
    element: "ice",
    rank: 22,
    target: "All foes",
  },
  Mabufula: {
    cost: 1024,
    effect: "Medium dmg",
    element: "ice",
    rank: 16,
    target: "All foes",
  },
  "Madness Nails": {
    cost: 1020,
    effect: "Heavy dmg, 1-3 hits",
    element: "phy",
    rank: 21,
    target: "Multi foes",
  },
  "Madness Needle": {
    cost: 1011,
    effect: "Weak dmg, Panic",
    element: "gun",
    rank: 3,
    target: "1 foe",
  },
  Magaon: {
    cost: 1010,
    effect: "Removes Smirk",
    element: "sup",
    rank: 7,
    target: "1 foe",
  },
  "Magic Torrent": {
    cost: 1050,
    effect: "Heavy dmg, 4-6 hits",
    element: "alm",
    rank: 0,
    target: "Multi foes",
  },
  Mahama: {
    cost: 1014,
    effect: "Weak dmg, Smirk: 30% kill",
    element: "lig",
    rank: 9,
    target: "All foes",
  },
  Mahamaon: {
    cost: 1028,
    effect: "Heavy dmg, Smirk: 55% kill",
    element: "lig",
    rank: 18,
    target: "All foes",
  },
  Makajam: {
    cost: 1004,
    effect: "Mute",
    element: "ail",
    rank: 15,
    target: "1 foe",
  },
  Makajamaon: {
    cost: 1015,
    effect: "Mute",
    element: "ail",
    rank: 23,
    target: "All foes",
  },
  Makakaja: {
    cost: 1020,
    effect: "Buff magic attack once",
    element: "sup",
    rank: 8,
    target: "All allies",
  },
  Makarabreak: {
    cost: 1035,
    effect: "Removes Makarakarn",
    element: "sup",
    rank: 30,
    target: "All foes",
  },
  Makarakarn: {
    cost: 1050,
    effect: "Reflects next non-almighty magic attack",
    element: "sup",
    rank: 23,
    target: "All allies",
  },
  Mamudo: {
    cost: 1014,
    effect: "Weak dmg, Smirk: 30% kill",
    element: "dar",
    rank: 9,
    target: "All foes",
  },
  Mamudoon: {
    cost: 1028,
    effect: "Heavy dmg, Smirk: 55% kill",
    element: "dar",
    rank: 18,
    target: "All foes",
  },
  "Mana Aid": {
    effect: "Restore large amount of MP after battle",
    element: "pas",
    rank: 15,
  },
  "Mana Bonus": {
    effect: "Maximum MP +10%",
    element: "pas",
    rank: 2,
  },
  "Mana Gain": {
    effect: "Maximum MP +20%",
    element: "pas",
    rank: 13,
  },
  "Mana Surge": {
    effect: "Maximum MP +30%",
    element: "pas",
    rank: 20,
  },
  Maragi: {
    cost: 1012,
    effect: "Weak dmg",
    element: "fir",
    rank: 5,
    target: "All foes",
  },
  Maragidyne: {
    cost: 1048,
    effect: "Heavy dmg",
    element: "fir",
    rank: 22,
    target: "All foes",
  },
  Maragion: {
    cost: 1024,
    effect: "Medium dmg",
    element: "fir",
    rank: 16,
    target: "All foes",
  },
  "Marin Karin": {
    cost: 1005,
    effect: "Charm",
    element: "ail",
    rank: 2,
    target: "1 foe",
  },
  Mazan: {
    cost: 1012,
    effect: "Weak dmg",
    element: "for",
    rank: 5,
    target: "All foes",
  },
  Mazandyne: {
    cost: 1048,
    effect: "Heavy dmg",
    element: "for",
    rank: 22,
    target: "All foes",
  },
  Mazanma: {
    cost: 1024,
    effect: "Medium dmg",
    element: "for",
    rank: 16,
    target: "All foes",
  },
  Mazio: {
    cost: 1012,
    effect: "Weak dmg",
    element: "ele",
    rank: 5,
    target: "All foes",
  },
  Maziodyne: {
    cost: 1048,
    effect: "Heavy dmg",
    element: "ele",
    rank: 22,
    target: "All foes",
  },
  Mazionga: {
    cost: 1024,
    effect: "Medium dmg",
    element: "ele",
    rank: 16,
    target: "All foes",
  },
  "Me Patra": {
    cost: 1018,
    effect: "Cures Sleep/Bind/Charm/Daze/Mute",
    element: "rec",
    rank: 15,
    target: "All Allies",
  },
  Media: {
    cost: 1024,
    effect: "Restores small amount of HP",
    element: "rec",
    rank: 12,
    target: "All allies",
  },
  Mediarahan: {
    cost: 1090,
    effect: "Restores all HP",
    element: "rec",
    rank: 27,
    target: "All allies",
  },
  Mediarama: {
    cost: 1048,
    effect: "Restores large amount of HP",
    element: "rec",
    rank: 23,
    target: "All allies",
  },
  "Megaton Press": {
    cost: 1020,
    effect: "Medium dmg, High crit/low hit",
    element: "phy",
    rank: 17,
    target: "All foes",
  },
  "Megiddo Ark": {
    cost: 1060,
    effect: "Severe dmg",
    element: "alm",
    rank: 0,
    target: "1 foe",
  },
  Megido: {
    cost: 1025,
    effect: "Weak dmg",
    element: "alm",
    rank: 18,
    target: "All foes",
  },
  Megidola: {
    cost: 1045,
    effect: "Medium dmg",
    element: "alm",
    rank: 23,
    target: "All foes",
  },
  Megidolaon: {
    cost: 1070,
    effect: "Heavy dmg",
    element: "alm",
    rank: 28,
    target: "All foes",
  },
  "Mist Rush": {
    cost: 1028,
    effect: "Medium dmg, 2-4 hits, Daze",
    element: "phy",
    rank: 27,
    target: "Multi foes",
  },
  "Mortal Jihad": {
    cost: 1014,
    effect: "Heavy dmg, High crit/low hit",
    element: "phy",
    rank: 21,
    target: "1 foe",
  },
  Mudo: {
    cost: 1006,
    effect: "Weak dmg, Smirk: 30% kill",
    element: "dar",
    rank: 3,
    target: "1 foe",
  },
  Mudoon: {
    cost: 1012,
    effect: "Heavy dmg, Smirk: 55% kill",
    element: "dar",
    rank: 13,
    target: "1 foe",
  },
  "Myriad Arrows": {
    cost: 1028,
    effect: "Medium dmg, 2-4 hits",
    element: "gun",
    rank: 27,
    target: "Multi foes",
  },
  "Needle Shot": {
    cost: 1004,
    effect: "Weak dmg",
    element: "gun",
    rank: 1,
    target: "1 foe",
  },
  Needlestorm: {
    cost: 1030,
    effect: "Medium dmg, Daze",
    element: "for",
    rank: 0,
    target: "All foes",
  },
  "Nihil Claw": {
    cost: 1011,
    effect: "Heavy dmg, 1-3 hits",
    element: "phy",
    rank: 19,
    target: "1 foe",
  },
  "Null Dark": {
    effect: "Nullifies dark damage",
    element: "pas",
    rank: 24,
  },
  "Null Elec": {
    effect: "Nullifies elec damage",
    element: "pas",
    rank: 24,
  },
  "Null Fire": {
    effect: "Nullifies fire damage",
    element: "pas",
    rank: 24,
  },
  "Null Force": {
    effect: "Nullifies force damage",
    element: "pas",
    rank: 24,
  },
  "Null Gun": {
    effect: "Nullifies gun damage",
    element: "pas",
    rank: 28,
  },
  "Null Ice": {
    effect: "Nullifies ice damage",
    element: "pas",
    rank: 24,
  },
  "Null Light": {
    effect: "Nullifies light damage",
    element: "pas",
    rank: 24,
  },
  "Null Mind": {
    effect: "Grants immunity to sleep, panic, charm and mute",
    element: "pas",
    rank: 18,
  },
  "Null Nerve": {
    effect: "Grants immunity to poison, sick, bind and daze",
    element: "pas",
    rank: 18,
  },
  "Null Phys": {
    effect: "Nullifies physical damage",
    element: "pas",
    rank: 28,
  },
  "Oni-Kagura": {
    cost: 1014,
    effect: "Medium dmg, High crit/low hit",
    element: "phy",
    rank: 7,
    target: "1 foe",
  },
  "Pandemic Bomb": {
    cost: 1015,
    effect: "Sick",
    element: "ail",
    rank: 10,
    target: "All foes",
  },
  "Panic Voice": {
    cost: 1020,
    effect: "Panic",
    element: "ail",
    rank: 10,
    target: "All foes",
  },
  Patra: {
    cost: 1006,
    effect: "Cures Sleep/Bind/Charm/Daze/Mute",
    element: "rec",
    rank: 2,
    target: "1 ally",
  },
  Pestilence: {
    cost: 1050,
    effect: "Weak dmg, Pierce, Strong vs ailment",
    element: "phy",
    rank: 0,
    target: "All foes",
  },
  "Phys Pierce": {
    effect: "Physical attacks bypass resistances",
    element: "pas",
    rank: 31,
  },
  "Phys Pleroma": {
    effect: "Phys damage +25%",
    element: "pas",
    rank: 25,
  },
  Poisma: {
    cost: 1004,
    effect: "Poison",
    element: "ail",
    rank: 2,
    target: "1 foe",
  },
  "Poison Breath": {
    cost: 1015,
    effect: "Poison",
    element: "ail",
    rank: 10,
    target: "All foes",
  },
  "Poison Claw": {
    cost: 1009,
    effect: "Medium dmg, Poison",
    element: "phy",
    rank: 3,
    target: "1 foe",
  },
  Posumudi: {
    cost: 1006,
    effect: "Cures Poison/Sick",
    element: "rec",
    rank: 2,
    target: "1 ally",
  },
  "Power Punch": {
    cost: 1009,
    effect: "Medium dmg, Smirk: Daze",
    element: "phy",
    rank: 7,
    target: "1 foe",
  },
  Pulinpa: {
    cost: 1005,
    effect: "Panic",
    element: "ail",
    rank: 2,
    target: "1 foe",
  },
  "Purple Smoke": {
    cost: 1022,
    effect: "Medium dmg, 1-3 hits, Panic",
    element: "phy",
    rank: 19,
    target: "Multi foes",
  },
  "Raging Blizzard": {
    cost: 1045,
    effect: "Severe dmg, Fire weakness",
    element: "ice",
    rank: 0,
    target: "1 foe",
  },
  "Raging Hellfire": {
    cost: 1045,
    effect: "Severe dmg, Ice weakness",
    element: "fir",
    rank: 0,
    target: "1 foe",
  },
  "Raging Lightning": {
    cost: 1045,
    effect: "Severe dmg, Force weakness",
    element: "ele",
    rank: 0,
    target: "1 foe",
  },
  "Raging Tempest": {
    cost: 1045,
    effect: "Severe dmg, Elec weakness",
    element: "for",
    rank: 0,
    target: "1 foe",
  },
  Ragnarok: {
    cost: 1060,
    effect: "Medium dmg, 1-4 hits",
    element: "fir",
    rank: 28,
    target: "Multi foes",
  },
  Rakukaja: {
    cost: 1020,
    effect: "Buff defense once",
    element: "sup",
    rank: 8,
    target: "All allies",
  },
  Rakunda: {
    cost: 1025,
    effect: "Debuff defense once",
    element: "sup",
    rank: 12,
    target: "All foes",
  },
  "Rapid Needle": {
    cost: 1005,
    effect: "Weak dmg",
    element: "gun",
    rank: 5,
    target: "All foes",
  },
  Recarm: {
    cost: 1020,
    effect: "Revive with half HP",
    element: "rec",
    rank: 12,
    target: "1 ally",
  },
  Recarmdra: {
    cost: 1006,
    effect: "Revive with full HP, cure all ailments, user dies",
    element: "rec",
    rank: 20,
    target: "All allies",
  },
  "Rending Claws": {
    cost: 1015,
    effect: "Heavy dmg, 3-4 hits",
    element: "phy",
    rank: 0,
    target: "1 foe",
  },
  "Repel Dark": {
    effect: "Reflects dark damage",
    element: "pas",
    rank: 29,
  },
  "Repel Elec": {
    effect: "Repels elec damage",
    element: "pas",
    rank: 29,
  },
  "Repel Fire": {
    effect: "Reflects fire damage",
    element: "pas",
    rank: 29,
  },
  "Repel Force": {
    effect: "Repels force damage",
    element: "pas",
    rank: 29,
  },
  "Repel Gun": {
    effect: "Repels gun damage",
    element: "pas",
    rank: 29,
  },
  "Repel Ice": {
    effect: "Repels ice damage",
    element: "pas",
    rank: 29,
  },
  "Repel Light": {
    effect: "Repels light damage",
    element: "pas",
    rank: 29,
  },
  "Repel Phys": {
    effect: "Repels physical damage",
    element: "pas",
    rank: 29,
  },
  "Resist Dark": {
    effect: "Resists dark damage",
    element: "pas",
    rank: 11,
  },
  "Resist Elec": {
    effect: "Resists elec damage",
    element: "pas",
    rank: 11,
  },
  "Resist Fire": {
    effect: "Resist fire damage",
    element: "pas",
    rank: 11,
  },
  "Resist Force": {
    effect: "Resists force damage",
    element: "pas",
    rank: 11,
  },
  "Resist Gun": {
    effect: "Resist gun damage",
    element: "pas",
    rank: 15,
  },
  "Resist Ice": {
    effect: "Resists ice damage",
    element: "pas",
    rank: 11,
  },
  "Resist Light": {
    effect: "Resists light damage",
    element: "pas",
    rank: 11,
  },
  "Resist Phys": {
    effect: "Resists physical damage",
    element: "pas",
    rank: 20,
  },
  Retaliate: {
    effect: "May perform a heavy attack when struck by a phys/gun attack",
    element: "pas",
    rank: 16,
  },
  "Riot Gun": {
    cost: 1030,
    effect: "Severe dmg, Smirk: Pierce",
    element: "gun",
    rank: 30,
    target: "1 foe",
  },
  "Ruinous Brand": {
    cost: 1020,
    effect: "Severe dmg",
    element: "phy",
    rank: 0,
    target: "1 foe",
  },
  Sabbatma: {
    cost: 1016,
    effect: "Summons an ally demon",
    element: "oth",
    rank: 13,
  },
  Salvation: {
    cost: 1100,
    effect: "Restores all HP, Smirk: Cures all ailments",
    element: "rec",
    rank: 30,
    target: "All allies",
  },
  Samarecarm: {
    cost: 1040,
    effect: "Revive with full HP",
    element: "rec",
    rank: 23,
    target: "1 allies",
  },
  "Scratch Dance": {
    cost: 1007,
    effect: "Weak dmg, 1-3 hits",
    element: "phy",
    rank: 9,
    target: "Multi foes",
  },
  "Sea of Chaos": {
    cost: 1050,
    effect: "Medium dmg, Strong vs Law",
    element: "alm",
    rank: 20,
    target: "All foes",
  },
  "Self-Righteous Vow": {
    effect: "Raises all stats once whenever own weakness is struck",
    element: "pas",
    rank: 0,
  },
  "Severe Judgement": {
    cost: 1050,
    effect: "Weak dmg, Smirk: Increased power",
    element: "alm",
    rank: 0,
    target: "All foes",
  },
  "Sexy Dance": {
    cost: 1020,
    effect: "Charm",
    element: "ail",
    rank: 10,
    target: "All foes",
  },
  Shibaboo: {
    cost: 1005,
    effect: "Bind",
    element: "ail",
    rank: 2,
    target: "1 foe",
  },
  "Shivering Taboo": {
    cost: 1040,
    effect: "Bind/Panic/Sleep/Sick/Poison",
    element: "ail",
    rank: 0,
    target: "All foes",
  },
  Shock: {
    cost: 1021,
    effect: "Weak dmg, 1-4 hits",
    element: "ele",
    rank: 9,
    target: "Multi foes",
  },
  "Silent Prayer": {
    cost: 1025,
    effect: "Removes all buffs/debuffs universally",
    element: "sup",
    rank: 17,
    target: "All allies",
  },
  "Smile Charge": {
    cost: 1050,
    effect: "Gains Smirk, condition: no ailments",
    element: "sup",
    rank: 27,
  },
  "Snake's Fangs": {
    cost: 1024,
    effect: "Heavy dmg, 2-3 hits, Bind",
    element: "gun",
    rank: 0,
    target: "Multi foes",
  },
  "Soul Divide": {
    cost: 1040,
    effect: "Heavy dmg, Mute",
    element: "alm",
    rank: 0,
    target: "All foes",
  },
  "Soul Drain": {
    cost: 1007,
    effect: "Weak dmg, Absorbs HP/MP, Smirk: Increased power",
    element: "alm",
    rank: 0,
    target: "1 foe",
  },
  "Spirit Drain": {
    cost: 1005,
    effect: "Weak dmg, Absorbs MP",
    element: "alm",
    rank: 13,
    target: "1 foe",
  },
  "Spring of Life": {
    effect: "Regain HP while walking. Inactive while in stock.",
    element: "pas",
    rank: 11,
  },
  "Strange Ray": {
    effect: "Lose 50% MP",
    element: "oth",
    rank: 0,
    target: "All foes",
  },
  "Stun Needle": {
    cost: 1010,
    effect: "Weak dmg, Bind",
    element: "gun",
    rank: 3,
    target: "1 foe",
  },
  "Stun Needles": {
    cost: 1022,
    effect: "Medium dmg, 1-3 hits, Bind",
    element: "gun",
    rank: 13,
    target: "Multi foes",
  },
  Sukukaja: {
    cost: 1020,
    effect: "Buff hit/evade once",
    element: "sup",
    rank: 8,
    target: "All allies",
  },
  Sukunda: {
    cost: 1025,
    effect: "Debuff hit/evade once",
    element: "sup",
    rank: 12,
    target: "All foes",
  },
  Tarukaja: {
    cost: 1020,
    effect: "Buff phys attack once",
    element: "sup",
    rank: 8,
    target: "All allies",
  },
  Tarunda: {
    cost: 1025,
    effect: "Debuff phys/magic attack once",
    element: "sup",
    rank: 12,
    target: "All foes",
  },
  "Tathlum Shot": {
    cost: 1007,
    effect: "Medium dmg",
    element: "gun",
    rank: 7,
    target: "1 foe",
  },
  Taunt: {
    cost: 1020,
    effect: "Tarukaja x2 + Makakaja x2 + Rakunda x2",
    element: "sup",
    rank: 15,
    target: "All foes",
  },
  "Tetanus Cut": {
    cost: 1011,
    effect: "Medium dmg, Sick",
    element: "phy",
    rank: 3,
    target: "1 foe",
  },
  Tetrabreak: {
    cost: 1035,
    effect: "Removes Tetrakarn",
    element: "sup",
    rank: 30,
    target: "All foes",
  },
  Tetraja: {
    cost: 1011,
    effect: "Nulls one Light or Dark attack",
    element: "sup",
    rank: 11,
    target: "All allies",
  },
  Tetrakarn: {
    cost: 1050,
    effect: "Reflects next phys/gun attack",
    element: "sup",
    rank: 21,
    target: "All allies",
  },
  "Thunder Gods": {
    cost: 1040,
    effect: "Severe dmg, Smirk: Pierce",
    element: "ele",
    rank: 29,
    target: "1 foe",
  },
  "Thunder Reign": {
    cost: 1070,
    effect: "Severe dmg",
    element: "ele",
    rank: 28,
    target: "All foes",
  },
  Titanomachia: {
    cost: 1035,
    effect: "Heavy dmg, High crit/low hit",
    element: "phy",
    rank: 21,
    target: "All foes",
  },
  "Toxic Sting": {
    cost: 1009,
    effect: "Weak dmg, Poison",
    element: "gun",
    rank: 3,
    target: "1 foe",
  },
  Trafuri: {
    cost: 1025,
    effect: "Guarantees escape from battle",
    element: "oth",
    rank: 6,
  },
  Trisagion: {
    cost: 1040,
    effect: "Severe dmg, Smirk: Pierce",
    element: "fir",
    rank: 29,
    target: "1 foe",
  },
  "True Zandyne": {
    cost: 1032,
    effect: "Severe dmg",
    element: "for",
    rank: 0,
    target: "1 foe",
  },
  "True Ziodyne": {
    cost: 1032,
    effect: "Severe dmg",
    element: "ele",
    rank: 0,
    target: "1 foe",
  },
  "Vengeful Thunder": {
    cost: 1025,
    effect: "Heavy dmg, Bind",
    element: "ele",
    rank: 0,
    target: "1 foe",
  },
  "Venomous Raga": {
    cost: 1065,
    effect: "Poison/Sick/Bind, Tarunda x2",
    element: "ail",
    rank: 0,
    target: "All foes",
  },
  "Victory Cry": {
    effect: "Restore full HP/MP after battle",
    element: "pas",
    rank: 28,
  },
  "War Cry": {
    cost: 1065,
    effect: "Tarunda + Rakunda",
    element: "sup",
    rank: 17,
    target: "All foes",
  },
  "Warding Shout": {
    cost: 1030,
    effect: "Prevents all ailments this turn",
    element: "sup",
    rank: 0,
    target: "All allies",
  },
  "Will of Flame": {
    cost: 1030,
    effect: "Fire pierce for this turn",
    element: "sup",
    rank: 28,
    target: "All allies",
  },
  "Will of Frost": {
    cost: 1030,
    effect: "Ice pierce for this turn",
    element: "sup",
    rank: 28,
    target: "All allies",
  },
  "Will of Thunder": {
    cost: 1030,
    effect: "Elec pierce for this turn",
    element: "sup",
    rank: 28,
    target: "All allies",
  },
  "Will of Wind": {
    cost: 1030,
    effect: "Force pierce for this turn",
    element: "sup",
    rank: 28,
    target: "All allies",
  },
  "Wind Breath": {
    cost: 1021,
    effect: "Weak dmg, 1-4 hits",
    element: "for",
    rank: 9,
    target: "Multi foes",
  },
  Workaholic: {
    effect: "Gain even more EXP from battles",
    element: "pas",
    rank: 16,
  },
  Zan: {
    cost: 1005,
    effect: "Weak dmg",
    element: "for",
    rank: 1,
    target: "1 foe",
  },
  Zandyne: {
    cost: 1020,
    effect: "Heavy dmg",
    element: "for",
    rank: 19,
    target: "1 foe",
  },
  Zanma: {
    cost: 1010,
    effect: "Medium dmg",
    element: "for",
    rank: 7,
    target: "1 foe",
  },
  Zio: {
    cost: 1005,
    effect: "Weak dmg",
    element: "ele",
    rank: 1,
    target: "1 foe",
  },
  Ziodyne: {
    cost: 1020,
    effect: "Heavy dmg",
    element: "ele",
    rank: 19,
    target: "1 foe",
  },
  Zionga: {
    cost: 1010,
    effect: "Medium dmg",
    element: "ele",
    rank: 7,
    target: "1 foe",
  },
};
